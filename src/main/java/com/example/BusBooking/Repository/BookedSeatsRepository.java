...
public interface BookedSeatsRepository extends JpaRepository<BookedTickets, Integer> {
    ...
    @Query(value = "select * from bus.bookedTickets where userId = :id and date = :todayDate and sTime < :currentTime",
    nativeQuery = true)
    List<BookedTickets> TripsOnLive(int id, String todayDate, String currentTime);

    @Query(value = "Select * from bus.bookedTickets where userId = :id and" +
